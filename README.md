...
程序会检查 GitHub Releases 中的新版本，并展示更新说明。你可以选择立即更新、打开发布页、稍后提醒，或对当前版本选择“此版本不再提醒”。

便携包内包含 `OWTranslatorLiteUpdater.exe`。自动下载失败时，可以手动把最新 `OWTranslatorLite-v*-portable-win-x64.zip` 放到外层 `OWTranslatorLite/` 文件夹，再运行 updater 一键替换。

遇到问题时，优先在主窗口点击“导出反馈包”，再通过“汇报 Bug”打开 GitHub Issue，并把反馈包 zip 拖到 Issue 中。反馈包会脱敏 API Key；只有开启“诊断模式”时才会包含高级 debug 日志。
...
The app checks GitHub Releases for new versions and shows the release notes before you decide whether to update. You can update now, open the release page, remind later, or ignore the current version until a newer one appears.

Portable packages include `OWTranslatorLiteUpdater.exe`. If automatic download fails, manually place the latest `OWTranslatorLite-v*-portable-win-x64.zip` in the outer `OWTranslatorLite/` folder and run the updater to replace the app.

For bug reports, click Export Feedback Package in the main window, then use Report Bug to open a GitHub Issue and attach the generated zip. The package redacts API keys; advanced debug logs are included only when diagnostic mode is enabled.
