...
项目目标很明确：在不打断游戏视野和操作节奏的前提下，让中文玩家能快速理解队友和对手的文字沟通。当前版本为 `0.2.0-beta.4-ui`，已经完成 Timeline 顺序对齐、多帧 OCR 共识、韩语 jamo 容错和 Apple Dark Console UI 改造。
...
3. 填写 API URL 和 API Key；DeepSeek 默认 URL 为 `https://api.deepseek.com`。
4. DeepSeek API 需要充值余额并按量计费，聊天翻译用量通常很小，实际费用很低。
5. 点击“获取模型”，选择模型，例如 `deepseek-v4-flash`。
6. 点击“选择翻译区域”，框选 OW 左侧聊天区域，尽量完整包含玩家名和正文。
7. 点击“开始”，译文会显示在翻译框中。
8. 如需固定显示译文，打开“常态显示翻译框”；如需拖动、缩放或滚动历史，关闭“鼠标穿透”。
...
The goal is practical and narrow: help Chinese players understand match chat without interrupting visibility or input flow. The current version is `0.2.0-beta.4-ui`, with Timeline-based ordering, multi-frame OCR consensus, Korean jamo tolerance, and an Apple Dark Console UI.
...
3. Enter the API URL and API key. DeepSeek defaults to `https://api.deepseek.com`.
4. DeepSeek API usage requires account balance and is billed by usage; chat translation usually costs very little.
5. Click Fetch Models and choose a model such as `deepseek-v4-flash`.
6. Select the OW chat region. Include complete player names and message text.
7. Click Start. Translations will appear in the overlay.
8. Enable always-show translation box if needed. Disable click-through when you want to drag, resize, or scroll the overlay.
